...
void processReceivedIngredients(String data) {
  ...
  // Selection sort spice array to ascending container number
  for (uint8_t c = 0; c < numSpicesOrdered - 1; c++) { // loop thru container numbers
    uint8_t minIndex = c; // index of minimum element
    uint8_t d;
    for (d = c + 1; d < numSpicesOrdered; d++) { // loop to find minimum element
      if (spiceArray[d][0].toInt() < spiceArray[minIndex][0].toInt()) { // compare container numbers
        minIndex = d; // update the index of the minimum element
      }
    }
    for (uint8_t e = 0; e < 2; e++) { // swap both container numbers
      String tempVal = spiceArray[c][d];
      spiceArray[c][e] = spiceArray[minIndex][e];
      spiceArray[minIndex][e] = tempVal;
    }
  }
